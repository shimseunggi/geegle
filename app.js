...
// ----------------------------
// 6W (육하원칙) + 예/아니오(“~는 ~야?”) 선처리
// ----------------------------
const STOP_QWORDS = new Set([
  '어디','어딨','어딨어','어디에','어디야',
  '언제','누구','누가','뭐','무엇','왜','어떻게','어케'
]);
...
// “남산은 어디있어/어디 있어/어딨어?”에서 ‘남산’만 뽑기
function extractWhereTopic(q){
  const s = cleanPhrase(q);

  // 1) "X(은/는/이/가) 어디(에) (있어/임/야/냐)" 형태
  let m = s.match(/^(.+?)(?:은|는|이|가)?\s*(?:어딨|어딨어|어디)\s*(?:에)?\s*(?:있|임|야|냐)?/i);
  if (m){
    const t = cleanPhrase(m[1]);
    if (t && !STOP_QWORDS.has(t)) return t;
  }

  // 2) "어디에 X 있어?" 같은 뒤집힌 형태(드물지만 안전장치)
  m = s.match(/(?:어딨|어딨어|어디)\s*(?:에)?\s*(.+?)\s*(?:있|임|야|냐)\b/i);
  if (m){
    const t = cleanPhrase(m[1]);
    if (t && !STOP_QWORDS.has(t)) return t;
  }
...
// “명지대는 명문대야?” → subject=명지대, pred=명문대 (끝의 ‘야/인가/입니까’ 제거)
function parseYesNo(q){
  const s = cleanPhrase(q);

  // 6W 섞이면 예/아니오로 처리하지 않음
  if (/(어디|언제|누구|누가|왜|어떻게|어케|무엇|뭐)/.test(s)) return null;

  // "X는 Y야?" / "X가 Y인가?" / "X는 Y 맞아?"
  const m = s.match(/^(.+?)(?:은|는|이|가)?\s*([^?]+?)\s*(?:이야|야|인가|입니까|맞니|맞나|맞아|냐|니)$/i);
  if (!m) return null;

  const subject = cleanPhrase(m[1]);
  const pred = cleanPhrase(m[2]);

  if (!subject || !pred) return null;
  if (STOP_QWORDS.has(subject) || STOP_QWORDS.has(pred)) return null;

  return { subject, pred };
}

// 육하원칙/예-아니오를 먼저 처리해서 “말 되는” 한 줄 답 만들기
function answerBySixW(raw){
  const s = cleanPhrase(raw);
  const low = lowerQ(s);

  // WHERE
  if (/(어딨|어딨어|어디\s*(?:에)?\s*(?:있|임|야|냐)?)/i.test(low)){
    const topic = extractWhereTopic(s);
    if (topic){
      return pickOne([
        `소인 아뢰오되, ${topic}${josa(topic,'이','가')} 어디 있는지는 알지 못하옵니다요.`,
        `소인, ${topic}${josa(topic,'을','를')} 찾으시나 소인이 아는 바가 없사옵니다요.`,
        `${topic}${josa(topic,'은','는')} 어디에 있사옵니까? 소인도 모르옵니다요.`
      ]);
    }
    return pickOne([
      "무엇의 위치를 묻는지 더 적어주시옵소서.",
      "어디라 하셨사오나, 무엇을 찾으시는지 알 수 없사옵니다요."
    ]);
  }

  // WHEN
  if (/(언제|몇\s*시|몇\s*월|몇\s*일|언제쯤)/i.test(low)){
    return pickOne([
      "시각은 알 길이 없사오니, 기준(날짜·상황)을 더 밝히시옵소서.",
      "언제를 묻는지 알겠사오나, 소인이 단정할 수 없사옵니다요."
    ]);
  }
  ...
  if (/(누구|누가)/i.test(low)){
    return pickOne([
      "그 자가 누구인지는 소인이 아뢰지 못하옵니다요.",
      "인물의 내력은 알지 못하옵니다요."
    ]);
  }

  // WHY
  if (/(왜|어째서|까닭)/i.test(low)){
    return pickOne([
      "까닭은 짐작만 있을 뿐, 확정할 수 없사옵니다요.",
      "연유를 단정할 근거가 없사옵니다요."
    ]);
  }

  // HOW
  if (/(어떻게|어케|방법|절차)/i.test(low)){
    return pickOne([
      "방법은 한 가지가 아니옵니다요. 조건을 더 적어주시옵소서.",
      "소인이 아는 방법도 있으나, 상황이 부족하옵니다요."
    ]);
  }

  // WHAT
  if (/(뭐|무엇|뭔지)/i.test(low)){
    return pickOne([
      "그것이 무엇인지는 소인이 아뢰지 못하옵니다요.",
      "무엇을 뜻하는지 정보가 부족하옵니다요."
    ]);
  }

  // YES/NO (“~는 ~야?”)
  const yn = parseYesNo(s);
  if (yn){
    ...
    return `소인 아뢰오되, ${subject}${josa(subject,'은','는')} ${pred}라 단정하기는 어렵사옵니다요.`;
  }
...
function answerBySixW(raw){
  const s = cleanPhrase(raw);
    // ✅ 6W/예-아니오 질문은 룰보다 먼저 처리 (말이 안 되는 답 방지)
  const sixWLine = answerBySixW(raw);
  if (sixWLine) return sixWLine;
